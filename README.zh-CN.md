...
中文界面在构建时生成；RouterOS 通信、权限判断、数据库、备份与服务端程序使用对应版本的官方实现。**本仓库没有路由器配置、账号、密码或 NAS 数据，也不会自动操作你的路由器。**

## 中文范围

- 登录、首次配置、导航、仪表盘、连接、网络、设置等主要页面的静态界面文案。
- 表单的名称、提示、确认操作与应用自带的资源字段说明。
- 本次扫描的 1,732 项中翻译 1,480 项，252 项为经审阅保留的技术名、单位、协议或示例。此数字只覆盖提取器识别的显示文案，**不代表全应用百分之百汉化**。
